...
func WaitForPendingNodeClaims(ctx context.Context, obj interface{}, kubeClient client.Client) error {
	...
	for i := range nodeClaims.Items {
		...
		if nodeClaimInstanceType {
			_, found := lo.Find(nodeClaims.Items[i].GetConditions(), func(condition status.Condition) bool {
				return condition.Type == karpenterv1.ConditionTypeInitialized && condition.Status == metav1.ConditionFalse
			})
	...
	for {
		...
		default:
			...
			_, conditionFound := lo.Find(nodeClaimObj.GetConditions(), func(condition status.Condition) bool {
				return condition.Type == karpenterv1.ConditionTypeInitialized &&
