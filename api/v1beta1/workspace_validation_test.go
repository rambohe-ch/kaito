...
func TestWorkspaceValidateScaleSubresource(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Non-scale subresource with preferred nodes (should be allowed)",
			workspace: &Workspace{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-workspace",
					Namespace: "default",
				...
				Inference: &InferenceSpec{
					Preset: &PresetSpec{PresetMeta: PresetMeta{Name: "test-validation"}},
				},
			},
			subresource: "",   // Regular validation, not subresource
			expectErrs:  true, // Expect error due to invalid preset name
		},
		{
			name: "Main resource with preferred nodes (should be allowed)",
			workspace: &Workspace{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-workspace",
					Namespace: "default",
			...
			subresource: "",
			expectErrs:  true, // Expect error due to environment setup issues
		},
	}
